...
public abstract class AbstractKeystore {
	...
	private boolean certificateMatchesAnyRecipient(X509Certificate cert, Collection<RecipientIdentifier> recipients) {
		...
		for (RecipientIdentifier rid : recipients) {
			if (recipientMatchesCertificate(rid, certSki, certIssuer, certSerial,cert)) {
				return true;
			}
		}
		return false;
	...
	private boolean recipientMatchesCertificate(RecipientIdentifier rid,
		...
		if (rid.getType() == RecipientIdentifier.Type.SUBJECT_KEY_IDENTIFIER) {
			...
			byte[] ridSki = rid.getSki();
			if (ridSki == null) {
				return false;
			}
			
			// SKI from ASN1
			if (certSki != null && Arrays.equals(ridSki, certSki)) {
				return true;
			}

			// fallback: CALCULATE SKI (SHA-1 pubkey)
			byte[] calculatedSki = calculateSubjectKeyIdentifier(cert);
			return calculatedSki != null && Arrays.equals(ridSki, calculatedSki);
	...
	/**
	 * RFC 3280 type 1 key identifier.
	 * @param cert
	 * @return RFC 3280 type 1 key identifier
	 */
	private byte[] calculateSubjectKeyIdentifier(X509Certificate cert) {
		try {
